...
impl Indexer {
    pub(crate) fn resolve_database_references(&mut self) {
        ...
        for (file_id, literals) in &self.index.raw_literals {
            ...
            if let Some(mod_id) = module_sym_id {
                for lit in literals {
                    ...
                    let words: Vec<&str> = clean_lit.split(|c: char| !c.is_alphanumeric() && c != '_').collect();
            ...
            for (receiver, _) in prints {
                for (table_name, &table_sym_id) in &schema_map {
                    ...
                    if receiver == table_name || receiver.ends_with(&format!(".{}", table_name)) ||
                       receiver_lower == table_lower || receiver_lower.ends_with(&format!(".{}", table_lower)) {
        ...
        for (src, tgt) in new_links {
            self.index.resolved_calls.entry(src).or_default().push(tgt);
        }
    }
        ...
        for (&sym_id, used_keys) in &self.index.symbol_config_refs {
            for key in used_keys {
                if let Some(def_ids) = self.index.config_definitions.get(key) {
                    for &target_sid in def_ids { new_links.push((sym_id, target_sid)); }
                }
            }
        }
    ...
    // Missing: resolve_literal_dependencies, resolve_shared_literals, resolve_namespace_imports, resolve_iac_links
    // (Migrated these but kept brief for this block, assume standard migration pattern)
                         ...
                         if let Some(alias) = &imp.alias {
                            if let Some(target_fid) = self.resolve_import_path(fid, &imp.source) {
                                if let Some(target_mod_name) = file_mod_map.get(&target_fid) {
                                    self.index.local_variable_types.entry(scope_id).or_default().insert(alias.clone(), target_mod_name.clone());
                                }
                            }
                         }
                    }
    ...
    pub(crate) fn resolve_literal_dependencies(&mut self) {
         ...
         let config_file_ids: HashSet<FileId> = self.index.files.iter()
            .filter(|(path, _)| { let p = path.to_lowercase(); p.ends_with("json") || p.ends_with(".env") })
        ...
        for (src_id, literal) in potential_links {
            if let Some(target_id) = self.resolve_import_path(src_id, &literal) {
                if src_id != target_id {
                    ...
                    if !deps.contains(&target_id) { deps.push(target_id); }
    ...
    pub(crate) fn resolve_iac_links(&mut self) {
                ...
                for key in config_keys {
                    if let Some(def_files) = env_var_definitions.get(key) {
                        for &def_file_id in def_files {
                            if sym.file_id != def_file_id { new_file_links.push((sym.file_id, def_file_id)); }
                        }
                    }
                }
            }
        ...
        for (file_id, imports) in &self.index.file_imports {
            for imp in imports { if imp.source.contains("aws-sdk") || imp.source.contains("boto3") { aws_s3_users.push(*file_id); } }
        }
