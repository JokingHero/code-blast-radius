...
impl Indexer {
    pub(crate) fn resolve_database_references(&mut self) {
        ...
        // Snapshot: Collect data to avoid holding borrow on self.index.raw_literals
        let literals_snapshot: Vec<(FileId, Vec<String>)> = self.index.raw_literals
            .iter()
            .map(|(k, v)| (*k, v.clone()))
            .collect();
        ...
        // We iterate fingerprints, but we push to local new_links Vec, so no conflict yet
    ...
    pub(crate) fn resolve_file_dependencies(&mut self) {
        ...
        for fid in fids {
            ...
            // FIX: Clone imports to end borrow on self.index
    ...
    pub(crate) fn resolve_literal_dependencies(&mut self) {
        ...
        // FIX: Snapshot raw_literals to break borrow
        let literals_snapshot: Vec<(FileId, Vec<String>)> = self.index.raw_literals
            .iter()
            .map(|(k, v)| (*k, v.clone()))
            .collect();
    ...
    pub(crate) fn resolve_iac_links(&mut self) {
        ...
        // Snapshot config refs to iterate safely
        let config_refs_snapshot: Vec<(SymbolId, Vec<String>)> = self.index.symbol_config_refs
            .iter()
            .map(|(k, v)| (*k, v.clone()))
            .collect();
        ...
        for (sym_id, config_keys) in config_refs_snapshot {
            ...
            // This is safe because we aren't iterating `symbol_config_refs` directly anymore
