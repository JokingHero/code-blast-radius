...
impl Indexer {
    pub(crate) fn resolve_type_sniffing(&mut self) {
        ...
        for (&caller_id, receiver_map) in &self.staging.fingerprints {
            for (receiver_var_name, methods_called) in receiver_map {
                ...
                if let Some(hint) = type_hint {
                    ...
                    if hint.starts_with("returns:") {
                        if let Some(targets) = self.lookup.symbol_map.get(&hint[8..]) {
                    ...
                    if let Some(tn) = resolved_type_name {
                        ...
                        if let Some(type_symbol_ids) = self.lookup.symbol_map.get(clean_type_name) {
                            for &type_id in type_symbol_ids {
                                ...
                                if
                                    let Some(known_methods) = self.staging.container_methods.get(
                                        &type_id
                                    )
                                {
                                    for method_called in methods_called {
                                        ...
                                        if known_methods.contains(method_called) {
                                            if
                                                let Some(method_ids) =
                                                    self.lookup.symbol_map.get(method_called)
                                            {
    ...
    pub(crate) fn resolve_fingerprints(&mut self) {
        ...
        for (&cid, fprints) in &self.staging.fingerprints {
            for (receiver_var, meths) in fprints {
                ...
                for (&cont_id, cont_meths) in &self.staging.container_methods {
                ...
                for cont_id in final_candidates {
                    ...
                    if let Some(_cont_meths) = self.staging.container_methods.get(&cont_id) {
                        for m in meths {
                            if let Some(m_ids) = self.lookup.symbol_map.get(m) {
                                for &mid in m_ids {
    ...
    pub(crate) fn resolve_implicit_connections(&mut self) {
        ...
        let impls_snapshot = self.staging.raw_implementations.clone();

        for (cid, parents) in impls_snapshot {
            for p in parents {
                // Immutable borrow of lookup.symbol_map
                if let Some(pids) = self.lookup.symbol_map.get(&p) {
                    for &pid in pids {
