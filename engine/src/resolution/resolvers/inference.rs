...
impl Indexer {
    pub(crate) fn resolve_type_sniffing(&mut self) {
        ...
        // Iterate staging.fingerprints (immutable borrow of self.index)
        for (&caller_id, receiver_map) in &self.index.staging.fingerprints {
            for (receiver_var_name, methods_called) in receiver_map {
                let lookup_name = receiver_var_name
                    .trim_start_matches("this.")
                    .trim_start_matches("self.");

                // 1. Walk scope to find variable type using staging.local_variable_types
                ...
                if let Some(hint) = type_hint {
                    ...
                    if hint.starts_with("returns:") {
                        if let Some(targets) = self.index.lookup.symbol_map.get(&hint[8..]) {
                    ...
                    if let Some(tn) = resolved_type_name {
                        ...
                        if let Some(type_symbol_ids) = self.index.lookup.symbol_map.get(clean_type_name) {
                            for &type_id in type_symbol_ids {
                                new_links.push((caller_id, type_id, EdgeKind::TypeReference));
                                // Link to Methods from staging.container_methods
                                if
                                    let Some(known_methods) = self.index.staging.container_methods.get(
                                        &type_id
                                    )
                                {
                                    for method_called in methods_called {
                                        ...
                                        if known_methods.contains(method_called) {
                                            if
                                                let Some(method_ids) =
                                                    self.index.lookup.symbol_map.get(method_called)
                                            {
    ...
    pub(crate) fn resolve_fingerprints(&mut self) {
        ...
        // Iterate staging.fingerprints (immutable borrow)
        for (&cid, fprints) in &self.index.staging.fingerprints {
            for (receiver_var, meths) in fprints {
                ...
                for (&cont_id, cont_meths) in &self.index.staging.container_methods {
                ...
                for cont_id in final_candidates {
                    ...
                    if let Some(_cont_meths) = self.index.staging.container_methods.get(&cont_id) {
                        for m in meths {
                            if let Some(m_ids) = self.index.lookup.symbol_map.get(m) {
                                for &mid in m_ids {
    ...
    pub(crate) fn resolve_implicit_connections(&mut self) {
        ...
        // Snapshot the data we are iterating over from staging
        let impls_snapshot = self.index.staging.raw_implementations.clone();

        for (cid, parents) in impls_snapshot {
            for p in parents {
                // Immutable borrow of lookup.symbol_map
                if let Some(pids) = self.index.lookup.symbol_map.get(&p) {
                    for &pid in pids {
