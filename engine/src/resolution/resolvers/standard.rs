...
impl Indexer {
    pub(crate) fn resolve_external_imports(&mut self) {
        ...
        for (_file_id, imports) in &self.lookup.file_imports {
            for imp in imports {
                if
                    ...
                    !imp.source.starts_with("/")
                {
                    ...
                    let stub_exists = self.lookup.symbol_map.get(&sym_name).map_or(false, |ids| {
                        ids.iter().any(|&id| {
    ...
    pub(crate) fn resolve_function_calls(&mut self) {
        ...
        let entries: Vec<_> = self.staging.raw_calls
            .iter()
            .map(|(k, v)| (*k, v.clone()))
            .collect();
        ...
        for (caller_id, called_names) in entries {
            ...
            for name in called_names {
                ...
                if let Some(tid) = resolved_id {
                    ...
                    // This drops the borrow on self.lookup.symbol_map immediately.
                    let candidates: Vec<SymbolId> = self.lookup.symbol_map
                        .get(&name)
                        .map(|v| v.clone())
                        .unwrap_or_default();
    ...
    pub(crate) fn resolve_type_references(&mut self) {
        ...
        let entries: Vec<_> = self.staging.raw_type_refs
            .iter()
            .map(|(k, v)| (*k, v.clone()))
            .collect();
