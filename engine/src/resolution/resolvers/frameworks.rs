...
pub fn resolve_implicit_routes(
...
pub fn resolve_dependency_injection(
    ...
    for (scope_id, var_types) in &staging.local_variable_types {
        if let Some(scope_sym) = index.symbols.get(scope_id) {
            ...
            let is_constructor = config.map_or(false, |c|
                c.heuristics.constructor_names.contains(&scope_sym.name.as_str())
            );
    ...
    for (src_id, file_id, type_name) in injection_points {
        let type_sym_id_opt = core::resolve_single_call(
            index,
            lookup,
            path_map,
            id_map,
    ...
    for (src, target) in new_links {
        if src != target {
            add_edge(index, src, target, EdgeKind::Injects);
        }
    }
}
...
pub fn resolve_middleware_injection(
    ...
    for (hub_file_id, middleware_names) in &staging.raw_middleware_usage {
        ...
        for name in middleware_names {
            if
                let Some(symbol_id) = core::resolve_single_call(
                    index,
                    lookup,
                    path_map,
                    id_map,
        ...
        for imp in imports {
            if
                let Some(imported_file_id) = core::resolve_import_path(
                    index,
                    lookup,
                    path_map,
                    id_map,
                ...
                )
            {
                if
                    middleware_ids
                        ...
                        .any(
                            |&middleware_id|
                                index.symbols[&middleware_id].file_id == imported_file_id
                        )
                {
                    continue;
                }
                ...
                if let Some(target_id) = target_mod_id {
                    for &middleware_id in &middleware_ids {
                        new_links.push((target_id, middleware_id));
                    }
                }
            }
        }
