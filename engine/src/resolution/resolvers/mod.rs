...
impl Indexer {
    ...
    pub(crate) fn link_modules(&mut self, file_a: u32, file_b: u32) {
        let mod_a = self.index.symbols.values().find(|s| s.file_id == file_a && s.kind == "module").map(|s| s.id);
        let mod_b = self.index.symbols.values().find(|s| s.file_id == file_b && s.kind == "module").map(|s| s.id);
