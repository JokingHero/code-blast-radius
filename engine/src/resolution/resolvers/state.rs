...
pub fn resolve_pubsub_wildcards(
    ...
    for (file_id, literals) in &staging.raw_literals {
        ...
        for lit in literals {
            if lit.len() < 3 || !lit.contains(constants::PATH_SEPARATORS) {
                continue;
            }
    ...
    for (caller_id, receiver_map) in &staging.fingerprints {
        for (receiver_var, methods) in receiver_map {
            ...
            if let Some(class_id) = type_class_id {
                ...
                // Need to lookup file via ID now
                let file_path = index.files
                    .values()
                    .find(|f| f.id == file_id)
                ...
                if let Some(path) = file_path {
                    ...
                    if let Some(config) = configs.get(ext) {
                        if !config.heuristics.magic_methods.is_empty() {
                            let has_explicit = if
                                let Some(explicit_methods) = staging.container_methods.get(
                                    &class_id
                                )
                            {
                            ...
                            if !has_explicit {
                                if
                                    let Some(class_members) = staging.container_methods.get(
                                        &class_id
                                    )
                                {
                                    for &magic_name in config.heuristics.magic_methods {
                                        if class_members.contains(magic_name) {
                                            if
                                                let Some(candidates) =
                                                    lookup.symbol_map.get(magic_name)
                                            {
                                                for &magic_id in candidates {
                                                    if
                                                        index.symbols[&magic_id].parent_id ==
                                                        Some(class_id)
                                                    {
    ...
    for (caller_id, dec_names) in &staging.raw_decorators {
        ...
        for dec_name in dec_names {
            let clean = dec_name
                ...
                .unwrap_or(&dec_name)
                .trim()
                .trim_matches(constants::QUOTE_CHARS);
            if
                let Some(target_id) = core::resolve_single_call(
                    index,
                    lookup,
                    path_map,
                    id_map,
