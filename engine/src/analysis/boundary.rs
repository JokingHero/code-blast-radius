                ...
                match cap_name {
                    ...
                    "def" | "function.definition" => {
                        ...
                        kind = if k.contains("class") || k.contains("struct") || k.contains("record") || k.contains("object") || k.contains("model") || k.contains("enum") {
                             ...
                             SymbolKind::Class 
                        } else if k.contains("function") || k.contains("fn") || k.contains("method") || k.contains("macro") {
                             SymbolKind::Function
                        } else if k.contains("module") || k.contains("namespace") || k.contains("package") {
                             SymbolKind::Module
                            ...
                            for child in node.children(&mut child_cursor) {
                                let ck = child.kind();
                                if ck == "arrow_function" || ck == "function_expression" || ck == "call_expression" {
                                    is_fn_var = true; 
                                    break;
                                }
                                // Handle: variable_declarator -> value: arrow_function
                                if child.child_by_field_name("value").map(|v| v.kind().contains("function")).unwrap_or(false) {
                                    is_fn_var = true;
                                    break;
                                }
                            }
                            
                            if is_fn_var { SymbolKind::Function } else { SymbolKind::Variable }
