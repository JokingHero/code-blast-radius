...
fn test_typescript_call_chain() {
    ...
    // 2. Create Files
    fs::write(root_path.join("utils.ts"), r#"
        export function add(a: number, b: number) { return a + b; }
    "#).expect("Failed to write utils.ts");

    fs::write(root_path.join("main.ts"), r#"
    ...
    assert!(has_symbol(index, "calculateTotal"), "Function 'calculateTotal' should be indexed");
    ...
    let add_file_ids = index.symbol_map.get("add").expect("Symbol 'add' has no associated files");
    let utils_id = add_file_ids[0];

    // Get FileId for main.ts (via the 'calculateTotal' symbol)
    let calc_file_ids = index.symbol_map.get("calculateTotal").expect("Symbol 'calculateTotal' not found");
...
fn test_rust_symbol_kinds() {
    ...
    fs::write(root_path.join("lib.rs"), r#"
        struct MyData {
            id: u32
        }
    ...
    let manager = WorkspaceManager::new_in_memory(vec![root_path])
        .expect("Failed to initialize workspace");
    let index = &manager.index;
    ...
    let struct_def = file_struct.defs.iter().find(|d| d.name == "MyData").unwrap();
    ...
    assert!(
        ...
        "Expected MyData to be a Class or Interface, got {:?}", struct_def.kind
    );

    // Verify Function Kind
    let fn_ids = index.symbol_map.get("process_data").expect("process_data not found");
    let file_fn = index.files.get(&fn_ids[0]).unwrap();
    let fn_def = file_fn.defs.iter().find(|d| d.name == "process_data").unwrap();

    assert_eq!(fn_def.kind, SymbolKind::Function, "Expected process_data to be a Function");
}
