...
fn test_output_json_structure_and_merging() {
    ...
    // 3. Get Symbol IDs
    let id_a = indexer.index.symbol_map.get("funcA").expect("funcA missing")[0];
    let id_b = indexer.index.symbol_map.get("funcB").expect("funcB missing")[0];
    let id_c = indexer.index.symbol_map.get("funcC").expect("funcC missing")[0];
...
fn test_output_multi_file() {
    ...
    let id_main = indexer.index.symbol_map.get("main").unwrap()[0];
    let id_help = indexer.index.symbol_map.get("help").unwrap()[0];
