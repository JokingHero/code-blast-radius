...
fn test_output_json_structure() {
    ...
    assert_eq!(file_ctx.metadata.relevant_lines.len(), 1, "Expected 1 range covering the whole file");
...
fn test_output_multi_file() {
    ...
    fs::write(root_path.join("main.ts"), "import { help } from './utils'; function main() { help(); }").unwrap();
    ...
    // Build ID Map
    let mut id_map = HashMap::new();
    id_map.insert(id_main_file, root_path.join("main.ts").to_string_lossy().to_string());
    id_map.insert(id_help_file, root_path.join("utils.ts").to_string_lossy().to_string());
