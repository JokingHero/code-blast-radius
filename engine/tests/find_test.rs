...
fn test_fuzzy_find_symbols_and_files() {
    ...
    let manager = WorkspaceManager::new_in_memory(vec![root_path.clone()]).expect("Failed to init workspace");
    ...
    assert!(results.iter().any(|(name, _)| name == "authenticate_user"), "Failed to find authenticate_user");

    // Test Case 2: Fuzzy match for a symbol (auth_u)
    let results = fuzzy_search(&mut matcher, "auth_u");
    assert!(results.iter().any(|(name, _)| name == "authenticate_user"), "Failed fuzzy match auth_u");
    ...
    assert!(results.iter().any(|(name, _)| name.contains("auth_service.rs")), "Failed to find file auth_service.rs");

    // Test Case 4: Fuzzy match for a file (dt_mngr)
    let results = fuzzy_search(&mut matcher, "dt_mngr");
    assert!(results.iter().any(|(name, _)| name.contains("data_manager.py")), "Failed to fuzzy find data_manager.py");

    // Test Case 5: Class/Struct match
    let results = fuzzy_search(&mut matcher, "DataCache");
    assert!(results.iter().any(|(name, _)| name == "DataCache"), "Failed to find DataCache");
}
...
fn test_fuzzy_find_limit_and_sorting() {
    ...
    fs::write(root_path.join("test.rs"), "fn a1() {} fn a2() {} fn a3() {} fn b1() {}").unwrap();
