...
fn test_incremental_updates() {
    ...
    fs::write(workspace_path.join("utils.py"), "def helper():\n    pass")
...

    {
        ...
        assert!(
            ...
            "Old symbol should be present before rescan"
        );
        assert!(
            !has_func(&index, "init_system_v2"),
            "New symbol should not be present before rescan"
        );
        assert!(
            !has_func(&index, "helper"),
            "New file symbol should not be present before rescan"
        );
...
fn test_corrupted_index_recovery() {
    ...
    let index = persistence
        ...
        .expect("Should recover (return empty) from corruption, not panic");

    // Should result in a fresh, empty index
    assert!(index.files.is_empty(), " recovered index should be empty");
    assert!(
        index.symbol_map.is_empty(),
        " recovered symbol map should be empty"
    );
}
