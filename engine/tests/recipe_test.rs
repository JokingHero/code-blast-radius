...
use blast_radius_engine::recipes::models::{FileTransform, Recipe, RecipeOperation};
...
fn test_recipe_globbing_and_filtering() {
    ...
    fs::write(
        root_path.join("src/auth.test.ts"),
        "function test_login() {}",
    )
    .unwrap();
    ...
    let recipe = Recipe {
        ...
        description: None,
        operations: vec![
            RecipeOperation::AddFiles {
                pattern: "**/*.ts".to_string(),
            },
            RecipeOperation::RemoveFiles {
    ...
    // Paths are relative
    assert!(
        paths.iter().any(|p| p.ends_with("src/auth.ts")),
        "Should include auth.ts"
    );
    assert!(
        paths.iter().any(|p| p.ends_with("src/utils.ts")),
        "Should include utils.ts"
    );

    assert!(
        !paths.iter().any(|p| p.ends_with("src/auth.test.ts")),
        "Should exclude auth.test.ts"
    );
    assert!(
...
fn test_recipe_transformation_focus_mode() {
    ...
    transforms.insert(
        "logic.ts".to_string(),
        FileTransform::FocusOn(vec!["keepMe".to_string()]),
    );

    let recipe = Recipe {
        name: "Focus Logic".to_string(),
        description: None,
        operations: vec![RecipeOperation::AddFiles {
            pattern: "**/logic.ts".to_string(),
        }],
        transforms,
    };
    ...
    // 3. Assertions
    assert!(
        content.contains("function keepMe() {"),
        "Header for keepMe missing"
    );
    assert!(
        content.contains("return \"I am visible\""),
        "Body for keepMe should be visible"
    );

    assert!(
        content.contains("function hideMe()"),
        "Header for hideMe missing"
    );
    assert!(
        !content.contains("return \"I should be hidden\""),
        "Body for hideMe should be masked"
    );
...
fn test_recipe_drift_safety() {
    ...
    let mut manager =
        WorkspaceManager::new_in_memory(vec![root_path.clone()]).expect("Failed to init");
    ...
    transforms.insert(
        "drift.ts".to_string(),
        FileTransform::Skeletonize(vec!["target".to_string()]),
    );
    ...
    assert!(
        ...
        "Output should reflect updated file content"
    );
    assert!(
        content.contains("function target()"),
        "Function header should be present"
    );
    assert!(
        !content.contains("return \"modified\""),
        "Target body should be masked"
    );
