...
fn test_workspace_full_lifecycle() {
    ...
    let mut manager = WorkspaceManager::new(config_path.clone()).expect("Failed to init manager");
    ...
    fs::write(root_app.join("main.ts"), r#"
    ...
    let links_to_helper = edges.iter().any(|e| e.target_id == helper_ids[0] && e.kind == EdgeKind::Calls);
    ...
    fs::write(root_core.join("utils.ts"), "export function super_helper() { return 9000; }").unwrap();

    // Trigger Sync (Simulate "Refresh")
    manager.sync();

    // Assert: Old symbol gone, new symbol present
    assert!(!manager.indexer.lookup.symbol_map.contains_key("helper"), "Old symbol 'helper' should be gone");
    assert!(manager.indexer.lookup.symbol_map.contains_key("super_helper"), "New symbol 'super_helper' should exist");
    ...
    assert!(manager.indexer.lookup.symbol_map.contains_key("log"), "Newly added file should be indexed");
    ...
    assert!(!manager.indexer.lookup.symbol_map.contains_key("log"), "Deleted symbol should be removed");
    ...
    assert!(!manager.indexer.lookup.symbol_map.contains_key("main"), "Symbols from removed root should be purged");
    
    // Assert: Core symbols remain
    assert!(manager.indexer.lookup.symbol_map.contains_key("super_helper"), "Symbols from remaining root should persist");
