...
fn test_scenario_monorepo_nest_angular_prisma() {
    ...
    let ui_related = find_related_symbols(&indexer.index, &indexer.lookup, &indexer.reverse_graph, "AppComponent", None).unwrap();
    ...
    let api_related = find_related_symbols(&indexer.index, &indexer.lookup, &indexer.reverse_graph, "UsersController", None).unwrap();
    ...
    let dto_ids = indexer.lookup.symbol_map.get("UserDto").expect("UserDto should be indexed");
