...
function App() {
  ...
  // Auto-load previous session on app start
  onMount(() => {
    initSession();
  });

  // 1. Ad-Hoc Mode: Open a single directory
  ...
  // 2. Project Mode: Open a .cblast workspace file
    ...
    <div class="h-full w-full flex flex-col items-center justify-center relative overflow-hidden select-none bg-matrix-bg">
       ...
       <div class="z-10 flex flex-col gap-4 w-72">
          ...
          >
             ...
             <span class="relative z-10 opacity-50 group-hover:opacity-100 transition-opacity group-hover:translate-x-1 duration-300">→</span>
          </button>
          ...
          <Show when={state.isLoaded} fallback={<WelcomeScreen />}>
             ...
             <div class="flex-1 flex overflow-hidden">
                ...
                <div class="w-64 border-r border-matrix-border flex flex-col bg-black/40 relative z-50">
                    ...
                    <div class="h-12 shrink-0 flex items-center px-3 border-b border-matrix-border bg-matrix-panel select-none justify-between">
                        <div class="flex items-center">
                          ...
                          <span class="text-tiny font-bold tracking-wider opacity-70 text-matrix-primary">EXPLORER</span>
                        </div>
                        
                        {/* Mode Indicator */}
                        <div class="flex gap-1">
                            <Show when={state.config?.mode === 'project'}>
                                <span class="text-micro border border-matrix-border px-1 opacity-50 rounded bg-matrix-panel text-matrix-highlight" title="Project Mode">
                                    PROJ
                                </span>
                            </Show>
                            <Show when={state.config?.mode === 'unsaved-workspace'}>
                                <span class="text-micro border border-matrix-error px-1 rounded bg-matrix-error/10 text-matrix-error animate-pulse cursor-help" title="Unsaved Workspace">
                                    UNSAVED
                                </span>
                            </Show>
                        </div>
                ...
                <div class="flex-1 flex flex-col min-w-0 border-r border-matrix-border bg-matrix-bg relative">
                    ...
                    <div class="flex flex-row border-b border-matrix-border max-h-[40%] min-h-[180px] shrink-0 bg-matrix-bg/50">
                        <div class="flex-1 flex flex-col border-r border-matrix-border overflow-hidden relative">
                            <div class="sticky top-0 bg-matrix-bg/95 backdrop-blur z-10 px-4 py-1 border-b border-matrix-border/30 flex justify-between items-center h-8 shrink-0">
                                <span class="text-tiny uppercase tracking-widest opacity-40">Active Recipe</span>
                                <span class="text-tiny opacity-30 font-mono">{state.recipe.length.toString().padStart(2, '0')} ITEMS</span>
                            </div>
                    ...
                    <div class="flex-1 flex flex-col min-h-0 bg-matrix-panel/20">
                        <div class="sticky top-0 bg-matrix-panel/90 backdrop-blur z-10 px-4 py-1 border-b border-matrix-border/30 h-8 flex items-center shrink-0">
                            <span class="text-tiny uppercase tracking-widest opacity-40">Context Output Stream</span>
                        </div>
                ...
                <div class="w-64 bg-matrix-panel/50 flex flex-col">
                    <div class="h-12 shrink-0 flex items-center px-3 border-b border-matrix-border bg-matrix-panel select-none justify-between">
                        <span class="text-tiny font-bold tracking-wider opacity-70 text-matrix-primary">SAVED RECIPES</span>
                        <button class="text-tiny border border-matrix-border px-1 hover:bg-matrix-primary hover:text-matrix-bg transition">+</button>
                    </div>
                    <div class="flex-1 p-2 overflow-y-auto custom-scrollbar space-y-2">
                        {/* Placeholder for saved recipes */}
                    </div>
                </div>
              </div>
          </Show>
